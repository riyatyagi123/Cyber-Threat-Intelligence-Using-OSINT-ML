...
# ===============================
# PE FILE ML MODEL (NEW)
# ===============================
PE_MODEL_PATH = os.path.join(BASE_DIR, "pickle", "classifier.pkl")
PE_FEATURES_PATH = os.path.join(BASE_DIR, "pickle", "features.pkl")
...
API_KEYS = {
    'VT_API_KEY': 'api_key_here',
    'ABUSEIPDB_KEY': 'api_key_here',
    'GREYNOISE_KEY': 'api_key_here',
    'IPINFO_TOKEN': 'api_key_here',
    'SHODAN_API_KEY': 'api_key_here',
    'IPREGISTRY_KEY': 'api_key_here',
    'WHOISXMLAPI_KEY': 'api_key_here',
    'PULSEDIVE_KEY': 'api_key_here',
...
def vt_url():
    ...
    try:
        ...
        return jsonify({'error': str(e)}), 500
# Machine Learning URL Scanner
@app.route('/ml/url-scan', methods=['POST'])
def ml_url_scan():
    url = request.form.get('url')
    if not url:
        return jsonify({'error': 'URL required'}), 400
    ...
    try:
        ...
        result = {
            'service': 'ML URL Scanner',
            'url': url,
            'prediction': 'Safe' if prediction == 1 else 'Phishing',
            'confidence_safe': round(prob[1] * 100, 2),
            'confidence_phishing': round(prob[0] * 100, 2)
        }
...
def cyber_news():
    ...
    try:
        ...
        return jsonify({'error': str(e)}), 500
# ===============================
# Machine Learning PE File Scanner (NEW)
# ===============================
@app.route('/ml/file-scan', methods=['POST'])
def ml_file_scan():
    if 'file' not in request.files:
    ...
    try:
        # Extract PE features
        extracted_features = extract_pe_features(temp_path)

        if not extracted_features:
            return jsonify({'error': 'Invalid PE file or extraction failed'}), 400

        # Arrange features in correct order
        feature_vector = [extracted_features.get(f, 0) for f in pe_features]
        feature_vector = np.array(feature_vector).reshape(1, -1)

        prediction = int(pe_model.predict(feature_vector)[0])
        prob = pe_model.predict_proba(feature_vector)[0]

        result = {
            'service': 'ML PE File Scanner',
            'file_name': file.filename,
            'prediction': 'Malware' if prediction == 1 else 'Benign',
            'confidence_malware': round(prob[1] * 100, 2),
            'confidence_benign': round(prob[0] * 100, 2)
        }
